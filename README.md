...
一个面向数学建模竞赛与建模项目的 Codex Skill。它以题目覆盖、证据链、模型—数据—代码—论文一致性和当届规则合规为主线，协助完成从选题到提交审计的端到端工作。

> README 展示名称为“数学建模.skill”，Codex 内部调用名称保持为 `math-modeling`。GitHub 的仓库 URL 名称只接受 ASCII 字母、数字、句点、连字符和下划线，因此远程仓库名暂拟为 `math-modeling.skill`，等待发布前确认。

## 它解决什么问题

这个 Skill 不提供固定的“万能模型组合”，而是约束和组织完整的建模过程：
...
后续更新优先处理真实使用中暴露的问题，例如比赛时间箱、项目状态保存、结果一致性检查和提交包审计。不要为了增加模型数量或文档长度而扩充 Skill。

修改后至少执行：

1. Skill 结构验证；
2. Python 脚本语法与代表性样例测试；
3. 检查 `SKILL.md` 中的参考文件路径；
4. 检查 README、Skill 描述和实际能力是否一致；
5. 在真实赛题上进行阶段性或端到端验证。

## 许可证

当前待审版本尚未附加开源许可证。正式公开前需要决定他人是否可以复制、修改、再分发或用于商业场景，并据此选择合适的许可证。
