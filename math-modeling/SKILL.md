---
name: math-modeling
description: 数学建模竞赛与项目的端到端协作技能，兼顾论文手、建模手和代码手，并把论文手作为技术成果转论文时的默认主模式。用于用户提供赛题、附件、数据、领域资料、优秀论文、建模思路、代码、运行结果、图表或草稿，要求选题、拆题、调研、数据审计、模型设计、代码实现、结果验证、可视化、先讲懂模型与公式、设计章节目次、把队友成果整理为可人工深改的论文母稿、减少模板化表达、局部回修、论文审校、答辩准备或提交审计时。以题目覆盖、证据链、模型-数据-代码-论文一致性、最新认可稿和当届合规为主线，防止漏问、模型堆砌、数据泄漏、虚构结果或文献、旧稿回流、静默改变口径和只生成不运行的代码。
---
...
- 论文手任务先读 `references/paper-hand-workflow.md`，先完成理解、章节合同、标题树和证据槽，再生成正文；用户明确只要某一产物时停在对应阶段。
